...
const STATUS_FILTER_MAP: Record<string, object> = {
  open: { in: ["not_started", "in_progress"] },
  done: { equals: "done" },
  all: {},
};
...
export default async function TasksPage({
  ...
  const openCount = await db.task.count({
    where: { tenantId: TENANT_ID, status: { in: ["not_started", "in_progress"] }, parentTaskId: null },
  });
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-0.5">
          {[
            ...
            <Link
              key={key}
              href={`/tasks?status=${key}&due=${dueFilter}`}
        ...
        <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-0.5">
          {[
            ...
            <Link
              key={key}
              href={`/tasks?status=${statusFilter}&due=${key}`}
              className={`px-3 py-1 rounded-md text-sm transition-colors ${
                dueFilter === key
                  ? "bg-slate-900 text-white"
                  : "text-slate-600 hover:text-slate-900"
